...
![Logo](https://github.com/wibusoft-team/wibusoft/raw/b4ed358f7730dd1f1d8742d1fd67eea148625c08/example/banner.png)


## Installation

Install wibusoft with npm

```bash
npm install wibusoft
```
## How Use?

```javascript
const wibusoft = require('wibusoft')
```
# Game Feature
## Nickname Free Fire!

```js
wibusoft.game.nickNameFreefire('239814337').then(console.log)
```
 *return*
 ```txt
...
const array_str = await wibusoft.anime.getHotPic().then(console.log).catch(console.error)
// get raw url
await wibusoft.anime.getUrl(array_str[0]).then(console.log).catch(console.error)
...
```js
await wibusoft.anime.getCharacterHentaiByNameAndGetRawURL('keqing').then(console.log).catch(console.error) // gpp sd yang penting banyak
```
*return*
```txt
[
...
wibusoft.anime.getUrl(jangan[0]).then(console.log).catch(console.error)
```
*return*
```txt
https://cdn.2dhentai.club/wp-content/uploads/2022/05/Genshin-Impact-hentai-Keqing-feet-covered-in-cum-r34-scaled.jpg
...
async function gopay(idProduct, PhoneNumber) {
    const csrf = await wibusoft.payment.csrfGenerator()
    const { data } = await wibusoft.payment.listProduct(csrf)
    ...
    const gass = await wibusoft.payment.convertGopay(idProduct,PhoneNumber, csrf)
...
wibusoft.payment.convertSaldo(1, '085173222764').then(console.log).catch(console.error)
```
```txt
{
...
- [@wibusoft](https://www.github.com/wibusoft.id)
- [@hardianto](https://www.github.com/hardianto-cpu)

## Contributing

Contributions are always welcome!
 [Contributing Now](https://github.com/wibusoft-team/wibusoft)

 [Donate And Contact Me](https://saweria.co/flyo)

## Notice

if there is an error in the docs apologies. if there is an error, bug or request please contact me on create issue on github
